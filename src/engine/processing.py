...
def preprocess_users(df):
    ...
    df["sex"] = df["sex"].fillna("sex_unknown").replace({"Ðœ": "M", "Ð–": "F"}).astype("category")
...
def process_data(last_date):
    ...
    try:
        old_interactions = pd.read_sql(
            "SELECT user_id, item_id FROM interactions_processed",
            engine
        ).set_index(['user_id', 'item_id'])
    except Exception:
        old_interactions = pd.DataFrame()
    ...
    # Truncate and replace users/items tables
    ...
    # Append new interactions (index preserved as columns)
    interactions_df.to_sql('interactions_processed', engine, if_exists='append', index=True)
